...
func getKeyType(ctx *cli.Context) certcrypto.KeyType {
	...
	case "MLDSA44":
		return certcrypto.MLDSA44
	case "MLDSA65":
		return certcrypto.MLDSA65
	case "MLDSA87":
		return certcrypto.MLDSA87
	}
...
func getCertKeyType(ctx *cli.Context) certcrypto.KeyType {
	if ctx.IsSet("cert-key-type") {
		keyType := ctx.String("cert-key-type")
		switch strings.ToUpper(keyType) {
		case "MLDSA44":
			return certcrypto.MLDSA44
		case "MLDSA65":
