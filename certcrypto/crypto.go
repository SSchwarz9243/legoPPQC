...
func ParsePEMPrivateKey(key []byte) (crypto.PrivateKey, error) {
	...
	switch keyBlockDER.Type {
	case "ML-DSA-44 PRIVATE KEY":
		privKey := new(mldsa44.PrivateKey)
		if err := privKey.UnmarshalBinary(keyBlockDER.Bytes); err != nil {
			return nil, fmt.Errorf("failed to parse ML-DSA-44 private key: %w", err)
		}
		return privKey, nil
	...
	case "ML-DSA-87 PRIVATE KEY":
		privKey := new(mldsa87.PrivateKey)
		if err := privKey.UnmarshalBinary(keyBlockDER.Bytes); err != nil {
			return nil, fmt.Errorf("failed to parse ML-DSA-87 private key: %w", err)
		}
		return privKey, nil
	}
...
func createMLDSACSR(privateKey crypto.PrivateKey, opts CSROptions) ([]byte, error) {
	var (
		dnsNames    []string
		ipAddresses []net.IP
	)
	...
	for _, dns := range dnsNames {
		rawSANs = append(rawSANs, asn1.RawValue{Tag: 2, Class: 2, Bytes: []byte(dns)})
	}
	for _, ip := range ipAddresses {
		rawSANs = append(rawSANs, asn1.RawValue{Tag: 7, Class: 2, Bytes: ip})
	}

	sanBytes, err := asn1.Marshal(rawSANs)
	if err != nil {
		return nil, err
	}
	...
	// Get public key and algorithm OID based on key type
	var pubKeyBytes []byte
	var algOID asn1.ObjectIdentifier

	switch k := privateKey.(type) {
	case *mldsa44.PrivateKey:
		pub := k.Public().(*mldsa44.PublicKey)
		pubKeyBytes, err = pub.MarshalBinary()
		algOID = asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 17} // ML-DSA-44
	case *mldsa65.PrivateKey:
		pub := k.Public().(*mldsa65.PublicKey)
		pubKeyBytes, err = pub.MarshalBinary()
		algOID = asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 18} // ML-DSA-65
	case *mldsa87.PrivateKey:
		pub := k.Public().(*mldsa87.PublicKey)
		pubKeyBytes, err = pub.MarshalBinary()
		algOID = asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 19} // ML-DSA-87
	default:
		return nil, fmt.Errorf("unsupported ML-DSA key type: %T", privateKey)
	}
	if err != nil {
		return nil, err
	}
	...
	extsBytes, err := asn1.Marshal(extensions)
	if err != nil {
		return nil, err
	}
	...
	csrInfo := struct {
		...
		Attributes: asn1.RawValue{
			...
			Tag:        0,
			IsCompound: true,
			Bytes: mustMarshal(struct {
				OID   asn1.ObjectIdentifier
				Value asn1.RawValue
			}{
				OID:   asn1.ObjectIdentifier{1, 2, 840, 113549, 1, 9, 14},
				Value: asn1.RawValue{Class: 0, Tag: 17, IsCompound: true, Bytes: extsBytes},
			}),
		},
	}
	...
	case *mldsa44.PrivateKey:
		...
		if err := mldsa44.SignTo(k, csrInfoBytes, nil, false, sig); err != nil {
			return nil, err
		}
	case *mldsa65.PrivateKey:
		sig = make([]byte, mldsa65.SignatureSize)
		if err := mldsa65.SignTo(k, csrInfoBytes, nil, false, sig); err != nil {
			return nil, err
		}
	case *mldsa87.PrivateKey:
		sig = make([]byte, mldsa87.SignatureSize)
		if err := mldsa87.SignTo(k, csrInfoBytes, nil, false, sig); err != nil {
			return nil, err
		}
	}
	...
	// Build final CSR
	csr := struct {
		CertRequestInfo asn1.RawValue
		SignatureAlg    pkix.AlgorithmIdentifier
		Signature       asn1.BitString
	}{
		CertRequestInfo: asn1.RawValue{FullBytes: csrInfoBytes},
		SignatureAlg:    pkix.AlgorithmIdentifier{Algorithm: algOID},
...
func CreateCSR(privateKey crypto.PrivateKey, opts CSROptions) ([]byte, error) {
	...
	case *mldsa44.PrivateKey, *mldsa65.PrivateKey, *mldsa87.PrivateKey:
		return createMLDSACSR(privateKey, opts)
	}
...
func PEMBlock(data any) *pem.Block {
	...
	case *mldsa44.PrivateKey:
		...
		pemBlock = &pem.Block{Type: "ML-DSA-44 PRIVATE KEY", Bytes: keyBytes}
	case *mldsa65.PrivateKey:
		keyBytes, _ := key.MarshalBinary()
		pemBlock = &pem.Block{Type: "ML-DSA-65 PRIVATE KEY", Bytes: keyBytes}
	case *mldsa87.PrivateKey:
		keyBytes, _ := key.MarshalBinary()
		pemBlock = &pem.Block{Type: "ML-DSA-87 PRIVATE KEY", Bytes: keyBytes}
	}
